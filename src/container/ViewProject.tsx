...
const ViewProject: React.FC<ViewProjectProps> = () => {
  ...
  const currentProject = projects.find((project) => project.id === match);

  const [html, setHtml] = useState(currentProject.html);
  const [css, setCss] = useState(currentProject.css);
  const [js, setJs] = useState(currentProject.js);
  const [output, setOutput] = useState(currentProject.output);
